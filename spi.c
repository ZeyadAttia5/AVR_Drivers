...
SPI_tenuErrorStatus SPI_enuMasterInit(u8 Copy_u8DataMode, u8 Copy_u8ClockSelect, u8 Copy_u8DataOrder){
	...
	case SPI_Prescaler4:
		CLR_BIT(SPSR, SPSR_SPI2X); CLR_BIT(SPCR, SPCR_SPR1); CLR_BIT(SPCR, SPCR_SPR1);
		break;
	case SPI_Prescaler16:
		CLR_BIT(SPSR, SPSR_SPI2X); CLR_BIT(SPCR, SPCR_SPR1); SET_BIT(SPCR, SPCR_SPR1);
		break;
	case SPI_Prescaler64:
		CLR_BIT(SPSR, SPSR_SPI2X); SET_BIT(SPCR, SPCR_SPR1); CLR_BIT(SPCR, SPCR_SPR1);
		break;
	case SPI_Prescaler128:
		CLR_BIT(SPSR, SPSR_SPI2X); SET_BIT(SPCR, SPCR_SPR1); SET_BIT(SPCR, SPCR_SPR1);
		break;
	case SPI_2xPrescaler2:
		SET_BIT(SPSR, SPSR_SPI2X); CLR_BIT(SPCR, SPCR_SPR1); CLR_BIT(SPCR, SPCR_SPR1);
		break;
	case SPI_2xPrescaler8:
		SET_BIT(SPSR, SPSR_SPI2X); CLR_BIT(SPCR, SPCR_SPR1); SET_BIT(SPCR, SPCR_SPR1);
		break;
	case SPI_2xPrescaler32:
		SET_BIT(SPSR, SPSR_SPI2X); SET_BIT(SPCR, SPCR_SPR1); CLR_BIT(SPCR, SPCR_SPR1);
		break;
	case SPI_2xPrescaler64:
		SET_BIT(SPSR, SPSR_SPI2X); SET_BIT(SPCR, SPCR_SPR1); SET_BIT(SPCR, SPCR_SPR1);
		break;
	default:
		LOC_enuErrorStatus = spi_invalidInput;
		return LOC_enuErrorStatus;
